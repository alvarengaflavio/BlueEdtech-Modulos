...
console.log('#######################################');
console.log('\nNotas disponíveis em [BRL]:  1, 5, 10, 50 e 100');
console.log(
    '\nOpção [SAQUE] selecionada\nValor de Saque[BRL]: Mínimo [10] - Máximo [600]\n',
);

while (isNaN(saque)) {
    saque = parseInt(prompt('Valor do Saque em BRL: '));
}
valor = saque;
...
if (saque >= 10 && saque <= 600) {
    ...
    for (let i = 0; i < notas_maquina.length; i++) {
        ...
        if (element != 0) {
            console.log(
                `Sacada(s) ${element} nota(s) de ${notas_maquina[i]} [BRL]`,
            );
        }
    }

    console.log(
        `\nSaque de ${notas_saque.reduce(
            (a, b) => a + b,
            0,
        )} notas, totalizando R$ ${valor.toFixed(2)}`,
    );
    console.log('\nA BLU-BANK agradece sua preferência\nVolte Sempre =)\n\n');
} else {
